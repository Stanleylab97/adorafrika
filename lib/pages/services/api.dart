...
import 'package:adorafrika/helpers/format.dart';
...
class SaavnAPI {
  ...
  String baseUrl = 'www.backend.adorafrika.com';
  String apiStr = '/api';
  ...
  Future<List<Map>> fetchSearchResults(String searchQuery) async {
    ...
    if (res.statusCode == 200) {
      ...
      print('Inside songs: ${getMain["songs"]}');
  ...
  Future<Map> fetchPlaylistSongs(String playlistId) async {
    final String params = '${endpoints["playlistDetails"]}/$playlistId';
    try {
      final res = await getResponse(params);
      if (res.statusCode == 200) {
        final getMain = json.decode(res.body);
        if (getMain['samerythme'] != '') {
          final List responseList = getMain['samerythme'] as List;
          return {
            'samerythme': responseList,
            'error': '',
          };
        }
        return {
          'samerythme': List.empty(),
          'error': '',
        };
      } else {
        return {
